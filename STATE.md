...
| Live debug logs — every `console.*` is mirrored into a 500-entry ring buffer + streamed (SSE) to a **/logs.html** viewer (level filter, text search, pause/auto-scroll, clear); plus targeted structured logs for floor edits and AI calls; token-gated via `LOGS_TOKEN` (localhost open in dev) | ✅ | realtime `/logs/stream`+`/logs/recent`, `apps/web/logs.html` |
