...
| In-office floor editor — owner/admin drag-move, add, delete furniture + notes/timers/portals live; **multi-select** (Shift-click → group drag/delete), **undo/redo** (Ctrl+Z / Ctrl+Y + ↶↷ buttons, exact via client-id adds + server `restore` op), **floor layout templates** (office / lounge / classroom / event hall — one-click `Apply layout`, undoable via `setFurniture`), **room-drawing** (▭ Room → drag a rectangle → names a new room zone, undoable via `removeRoom`), grid-snapped; RBAC-gated; **persists** across restarts (Redis or JSON file) | ✅ | realtime `editFloor` (+`restore`/`template`/`setFurniture`/`addRoom`/`removeRoom`) + persist, web |
