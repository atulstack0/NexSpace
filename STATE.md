...
| Games (🎮) — **❄️ Freeze Tag** (floor-wide proximity chase: a random "It" freezes runners on contact, free runners thaw frozen teammates, frozen players can't move; win = everyone frozen or the 90s timer; live markers — 2D rings + 3D **ground rings** (red under "It", ice‑blue under frozen) and ❄️/🔴 name tags — plus a status banner with countdown; start/stop from the panel) **and** shared **tic-tac-toe** (X/O seats, turn-enforced, win/draw, reset) — both synced live | ✅ | realtime `tagStart`/`tagStop` + tick, `game*`, web |
...
| **GLB avatar** — `MODEL_URL` points at the uploaded `apps/web/models/pixellabs-glb-3347.glb` (swappable to any `.glb` / CDN url); auto-fits height, plays Idle/Walk clips if present, subtle per-person tint, **falls back to the sculpted styled avatar** on any load failure | ✅ | `apps/web/index.html`, `CHARACTER.md` |
...
| Office-like ground floor — partition walls forming cubicles + a reception/lobby divider + a walled **Meeting Room** (3rd room), open-plan **desk pods** w/ chairs (rotation-faced), reception desk, meeting table, plants; lounge in the corner; rendered in 2D + 3D | ✅ | realtime `makeDefaultFloor`, web |
...
| In-office floor editor — owner/admin drag-move, add, delete furniture + notes/timers/portals live; **multi-select** (Shift-click → group drag/delete), **undo/redo** (Ctrl+Z / Ctrl+Y + ↶↷ buttons, exact via client-id adds + server `restore` op), **rotate furniture** (⟳ button or R / Shift+R → 90° steps, cosmetic facing, undoable), **floor layout templates** (office / lounge / classroom / event hall — clean, room-aware arrangements, undoable via `setFurniture`), **room-drawing** (▭ Room → drag a rectangle → names a new room zone) **and full room editing** (click a room to select, drag to move — door moves with it — drag the corner/edge **resize handles**, **double-click to rename**, or 🗑 delete; all undoable) **and full wall editing** (▥ Wall → drag to draw a new wall; click a wall to select → drag to move / delete; floor-boundary walls protected); grid-snapped; RBAC-gated; **persists** across restarts (Redis or JSON file) | ✅ | realtime `editFloor` (+`restore`/`template`/`setFurniture`/`addRoom`/`moveRoom`/`resizeRoom`/`renameRoom`/`removeRoom`) + persist, web |
