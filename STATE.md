...
| Accessibility (§7) — keyboard focus-visible outlines on all controls, screen-reader `aria-label`s mirrored from tooltips, control rails marked up as `toolbar`s, the world canvas labelled + focusable, and a polite `aria-live` region that announces toasts/status to screen readers | ✅ | `apps/web/index.html` |
| In-office floor editor — owner/admin drag-move, add, delete furniture + notes/timers/portals live; **multi-select** (Shift-click → group drag/delete), **undo/redo** (Ctrl+Z / Ctrl+Y + ↶↷ buttons, exact via client-id adds + server `restore` op), **floor layout templates** (office / lounge / classroom / event hall — one-click `Apply layout`, undoable via `setFurniture`), **room-drawing** (▭ Room → drag a rectangle → names a new room zone) **and full room editing** (click a room to select, drag to move — door moves with it — drag the corner/edge **resize handles**, or 🗑 delete; all undoable), grid-snapped; RBAC-gated; **persists** across restarts (Redis or JSON file) | ✅ | realtime `editFloor` (+`restore`/`template`/`setFurniture`/`addRoom`/`moveRoom`/`resizeRoom`/`removeRoom`) + persist, web |
