...
| In-office floor editor — owner/admin drag-move, add, delete furniture + notes/timers/portals live; **multi-select** (Shift-click → group drag/delete), **undo/redo** (Ctrl+Z / Ctrl+Y + ↶↷ buttons, exact via client-id adds + server `restore` op), **floor layout templates** (office / lounge / classroom / event hall — one-click `Apply layout`, undoable via `setFurniture`), **room-drawing** (▭ Room → drag a rectangle → names a new room zone) **and full room editing** (click a room to select, drag to move — door moves with it — drag the corner/edge **resize handles**, **double-click to rename**, or 🗑 delete; all undoable) **and editable walls** (click a wall segment to select → drag to move / delete; floor-boundary walls protected); grid-snapped; RBAC-gated; **persists** across restarts (Redis or JSON file) | ✅ | realtime `editFloor` (+`restore`/`template`/`setFurniture`/`addRoom`/`moveRoom`/`resizeRoom`/`renameRoom`/`removeRoom`) + persist, web |
...
| Live debug logs — every server `console.*` is mirrored into a 500-entry ring buffer + streamed (SSE) to a **/logs.html** viewer (level filter, text search, pause/auto-scroll, clear); **browser errors + key client events** are forwarded over the socket and shown as `[client:<name>]`; plus targeted structured logs for floor edits and AI calls; token-gated via `LOGS_TOKEN` (localhost open in dev) | ✅ | realtime `/logs/stream`+`/logs/recent` + `clientlog`, `apps/web/logs.html` |
