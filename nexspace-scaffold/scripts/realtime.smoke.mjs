...
try {
  ...
  admin.ws.send(JSON.stringify({ t: "editFloor", op: "renameRoom", id: "r-test01", name: "Pods" }));
  await wait(210);
  (guest.world?.rooms?.find((r) => r.id === "r-test01")?.name === "Pods") ? ok("editFloor renameRoom renames a room") : bad("renameRoom did not rename");
